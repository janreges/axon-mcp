...
async fn test_mock_repository_state_transitions() {
    ...
    let updated_task = repo.set_state(task.id, TaskState::InProgress).await.unwrap();
    assert_eq!(updated_task.state, TaskState::InProgress);
    
    // Test invalid state transition
    let result = repo.set_state(task.id, TaskState::Archived).await;
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), TaskError::InvalidStateTransition(_, _)));
}
...
async fn test_fixtures_create_tasks_in_all_states() {
    ...
    let done_tasks: Vec<_> = tasks.iter().filter(|t| t.state == TaskState::Done).collect();
    ...
    for i in 0..10 {
        ...
        set.spawn(async move {
            let new_task = NewTaskBuilder::new()
                .with_code(&format!("CONCURRENT-{:03}", i))
...
async fn test_contract_tests_with_mock() {
    ...
    assert!(history.iter().any(|call| call.contains("get_by_id")), "Should have called get_by_id");
}