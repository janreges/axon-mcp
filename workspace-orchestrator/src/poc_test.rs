...
use anyhow::Result;
use std::fs;
use task_core::{workspace_setup::*, PrdDocument};
use tracing::info;
...
"#;
    ...
    let mock_workflow = AgenticWorkflowDescription {
        ...
        task_decomposition_strategy:
            "Sequential development with parallel testing and documentation".to_string(),
    ...
    info!(
        "✅ Mock workflow created with {} agents",
        mock_workflow.suggested_agents.len()
    );

    // Step 3: Test template rendering with R.I.C.H. pattern
    let template_content = fs::read_to_string("templates/CLAUDE.md.hbs").unwrap_or_else(|_| {
...
"#
    ...
    info!(
        "   Agents: {} (vs. hardcoded 8 agents)",
        mock_workflow.recommended_agent_count
    );
    info!(
        "   Template: {} characters generated",
        rendered_prompt.len()
    );
    ...
    if let Some(first_agent) = mock_workflow.suggested_agents.first() {
        let rich_prompt = template.create_rich_agent_prompt(
            first_agent,
            &prd,
            &mock_workflow.coordination_patterns,
        );
...
fn validate_rich_pattern(content: &str) -> Vec<(&'static str, bool)> {
    vec![
        (
            "Role-Specific",
            content.contains("YOUR ROLE") || content.contains("expertise"),
        ),
        (
            "Imperative",
            content.contains("MUST") || content.contains("SHALL") || content.contains("CRITICAL"),
        ),
        (
            "Contextual",
            content.contains("PROJECT") || content.contains("CONTEXT"),
        ),
        (
            "Handoff",
            content.contains("COMMUNICATION") || content.contains("COORDINATION"),
        ),
        (
            "Task Tool JSON",
            content.contains("agent_role_and_task") || content.contains("task_tool_json"),
        ),
        (
            "Agent Count",
            content.contains("3") || content.contains("recommended"),
        ),
        (
            "Template Variables",
            content.contains("Markdown to HTML") || content.contains("CLI"),
        ),
    ]
}
