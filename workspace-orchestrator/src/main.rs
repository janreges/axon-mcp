...
use std::fs;
use task_core::{
    workspace_setup::{SuggestedAgent, *},
    PrdDocument,
};
...
impl McpClient {
    ...
    pub async fn get_agentic_workflow_description(
        ...
        info!(
            "Successfully received workflow description with {} agents",
            workflow.suggested_agents.len()
        );
...
impl ClaudemdTemplate {
    ...
    pub fn render(
        ...
        let rendered = self
            .engine
            .render(&self.template_name, &template_data)
    ...
    pub fn create_rich_agent_prompt(
        &self,
...
**REMEMBER:** You are a senior expert in your field. Act with confidence, take ownership, and deliver exceptional results."#,
            ...
            project_overview = prd
                .overview
                .as_ref()
                .unwrap_or(&"No overview provided".to_string()),
...
async fn main() -> Result<()> {
    ...
    info!(
        "   Valid: {}, Objectives: {}, User Stories: {}, Tech Requirements: {}",
        prd.is_valid(),
        prd.objectives.len(),
    ...
    info!(
        "   Recommended Agents: {}",
        workflow.recommended_agent_count
    );
    ...
    let template_content = fs::read_to_string(&config.template_path).map_err(|e| {
        anyhow::anyhow!(
    ...
    info!(
        "💾 Writing generated control agent prompt to: {}",
        config.output_path
    );
    ...
    fs::write(&config.output_path, &rendered_prompt).map_err(|e| {
        anyhow::anyhow!(
    ...
    info!(
        "   Recommended Agents: {} (optimized for project complexity)",
        workflow.recommended_agent_count
    );
    ...
    info!("════════════════════════════════════════");
    info!(
        "📁 Generated control agent prompt saved to: {}",
        config.output_path
    );
    ...
    if rendered_prompt.len() > 500 {
        println!(
            "... (truncated, see {} for full content)",
            config.output_path
        );
    }
    println!("--- END PREVIEW ---\n");
