...
impl TaskRepository for FastMockRepository {
    ...
    async fn update(&self, id: i32, updates: task_core::UpdateTask) -> Result<Task> {
        Ok(Task {
            ...
            description: updates
                .description
                .unwrap_or_else(|| "Test description".to_string()),
            owner_agent_name: Some(
                updates
                    .owner_agent_name
    ...
    async fn set_state(&self, id: i32, state: TaskState) -> Result<Task> {
        Ok(Task {
            ...
            state,
            inserted_at: Utc::now(),
            done_at: if state == TaskState::Done {
                Some(Utc::now())
            } else {
                None
            },
...
async fn test_create_task_performance() {
    ...
    assert!(
        ...
        "Create task took {}ms, should be <100ms",
        duration.as_millis()
    );
...
async fn test_get_task_performance() {
    ...
    assert!(
        duration.as_millis() < 100,
        "Get task took {}ms, should be <100ms",
        duration.as_millis()
    );
...
async fn test_list_tasks_performance() {
    ...
    assert!(
        duration.as_millis() < 100,
        "List tasks took {}ms, should be <100ms",
        duration.as_millis()
    );
...
async fn test_update_task_performance() {
    ...
    assert!(
        duration.as_millis() < 100,
        "Update task took {}ms, should be <100ms",
        duration.as_millis()
    );
...
async fn test_state_transition_performance() {
    ...
    assert!(
        duration.as_millis() < 100,
        "Set state took {}ms, should be <100ms",
        duration.as_millis()
    );
...
async fn test_serialization_performance() {
    ...
    assert!(
        ...
        "Average serialization took {}μs, should be <1000μs",
        avg_duration.as_micros()
    );
    println!(
        "Average serialization performance: {}μs",
        avg_duration.as_micros()
    );
}
...
async fn test_concurrent_operations_performance() {
    ...
    assert!(
        ...
        "10 concurrent operations took {}ms, should be <100ms",
        total_duration.as_millis()
    );
    println!(
        "10 concurrent operations performance: {}ms",
        total_duration.as_millis()
    );
}
...
fn test_error_conversion_performance() {
    ...
    assert!(
        ...
        "Average error conversion took {}μs, should be <100μs",
        avg_duration.as_micros()
    );
    println!(
        "Average error conversion performance: {}μs",
        avg_duration.as_micros()
    );
}
