...
fn test_error_codes_compliance() {
    ...
    for (error, expected_code) in test_cases {
        ...
        assert!(
            ...
            "Error code {code} is not in the implementation-defined range"
        );
    }
}
...
fn test_parameter_deserialization() {
    ...
    assert_eq!(
        ...
        "Testing parameter deserialization"
    );
    assert_eq!(
        create_params.owner_agent_name.as_deref(),
        Some("param-tester")
    );
}
...
fn test_invalid_datetime_format() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        task_core::TaskError::Validation(_)
    ));
}
