use task_core::{AiToolType, PrdDocument, WorkspaceSetupService};
...
async fn test_scenario_simple_markdown_converter() {
    ...
    println!(
        "   Technical Requirements: {}",
        prd.technical_requirements.len()
    );
    ...
    println!(
        "   Recommended Agents: {}",
        workflow.payload.recommended_agent_count
    );
    println!(
        "   Actual Generated Agents: {}",
        workflow.payload.suggested_agents.len()
    );
    ...
    assert!(
        workflow.payload.recommended_agent_count <= 3,
        "Simple project should have ≤3 agents"
    );
    assert!(
        !workflow.payload.suggested_agents.is_empty(),
        "Should generate at least one agent"
    );
    assert!(
        workflow
            .payload
...
async fn test_scenario_complex_weather_app() {
    ...
    assert!(
        workflow.payload.recommended_agent_count >= 4,
        "Complex project should have ≥4 agents"
    );
    assert!(
        workflow.payload.suggested_agents.len() >= 3,
        "Should generate multiple agents"
    );
    assert!(
        !workflow.payload.coordination_patterns.is_empty(),
        "Complex project should have coordination patterns"
    );
    ...
    let has_frontend_agent = agent_names
        .iter()
        .any(|name| name.contains("frontend") || name.contains("ui") || name.contains("client"));
    let has_backend_agent = agent_names
        .iter()
        .any(|name| name.contains("backend") || name.contains("api") || name.contains("server"));

    assert!(
        has_frontend_agent || has_backend_agent,
...
async fn test_scenario_edge_vague_prd() {
    ...
    match workflow_result {
        Ok(workflow) => {
            ...
            assert!(
                workflow.payload.recommended_agent_count > 0,
                "Should still generate some agents"
            );
...
async fn test_scenario_edge_minimal_prd() {
    ...
    match prd_result {
        Ok(prd) => {
            ...
            assert!(
                !prd.get_validation_errors().is_empty(),
...
async fn test_all_mcp_functions_integration() {
    ...
    let setup_instructions = service
        .get_setup_instructions(AiToolType::ClaudeCode)
        .await
        .unwrap();
    assert_eq!(
        setup_instructions.payload.ai_tool_type,
    ...
    let main_instructions = service
        .get_main_file_instructions(AiToolType::ClaudeCode)
        .await
        .unwrap();
    ...
    let manifest = service
        .generate_workspace_manifest(&prd, &empty_agents, true)
        .await
        .unwrap();
