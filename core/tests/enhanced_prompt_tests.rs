...
use task_core::ai_tool_adapters::{AiToolAdapter, ClaudeCodeAdapter};
use task_core::{
    workspace_setup::{ProjectArchetype, SuggestedAgent},
...
fn test_different_archetypes_generate_different_prompts() {
    ...
    let cli_prompt =
        builder.generate_agent_prompt(&cli_agent, &ProjectArchetype::CliTool, "CLI project", None);
...
async fn test_claude_code_adapter_enhanced_setup() {
    ...
    let step_names: Vec<&str> = setup_instructions
        .setup_steps
        .iter()
    ...
    assert!(step_names
        .iter()
        .any(|name| name.contains("MCP Connection")));
    assert!(step_names
        .iter()
        .any(|name| name.contains("capability catalog")));
    assert!(step_names.iter().any(|name| name.contains("enhanced")));

    // Verify enhanced MCP functions are included
    let function_names: Vec<&str> = setup_instructions
...
fn test_agent_contract_structure() {
    ...
    let prompt =
        builder.generate_agent_prompt(&agent, &ProjectArchetype::Generic, "Test project", None);
...
fn test_prompt_contains_all_2025_features() {
    ...
    assert!(
        prompt.contains("target_agent_name")
            || prompt.contains("author_agent_name")
