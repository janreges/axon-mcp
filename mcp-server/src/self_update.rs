...
fn detect_platform() -> Result<String> {
    ...
    let platform = match (os, arch) {
        ...
        ("macos", _) | ("darwin", _) => "universal-apple-darwin",
...
async fn perform_update(download_url: &str) -> Result<()> {
    ...
    fs::write(&temp_file, bytes).context("Failed to write update to temp file")?;
...
fn extract_binary(archive_path: &Path) -> Result<PathBuf> {
    ...
    if archive_path.extension().and_then(|s| s.to_str()) == Some("zip") {
        ...
        #[cfg(not(windows))]
        return Err(anyhow::anyhow!(
            "ZIP extraction not supported on this platform"
        ));
    } else {
